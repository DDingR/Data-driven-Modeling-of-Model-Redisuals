function report_list = prediction_check_each(PLOT_DATA, seed, NN_NAME, FILE_NAME, TEST_NUM, Ts, Np)
    close all
...
%         NN_NAME = "0508_0339PM/21000"; % 0507_0746PM
        NN_NAME = "0508_0656PM/FINAL100000"; % 0507_0746PM
...
%         PLOT_DATA = false;
    ...
    nnx = "./savemodel/X" + NN_NAME + ".onnx";
    nnx = importONNXNetwork( ...
      nnx,  TargetNetwork="dlnetwork", InputDataFormats="BC", OutputDataFormats="BC" ...
    );
    nny = "./savemodel/Y" + NN_NAME + ".onnx";
    nny = importONNXNetwork( ...
      nny,  TargetNetwork="dlnetwork", InputDataFormats="BC", OutputDataFormats="BC" ...
    );
    nnr = "./savemodel/R" + NN_NAME + ".onnx";
    nnr = importONNXNetwork( ...
      nnr,  TargetNetwork="dlnetwork", InputDataFormats="BC", OutputDataFormats="BC" ...
    );    
    % analyzeNetwork(nn)
...
%         input_sample = dlarray(sample/norm(sample), "CB");
        ...
        gx0 = extractdata(predict(nnx, input_sample));    
        gy0 = extractdata(predict(nny, input_sample));        
        gr0 = extractdata(predict(nnr, input_sample));        
        ...
        [v,grad] = dlfeval(@model,nnx,input_sample);
        tmp = extractdata(grad); % input_sample = extractdata(input_sample);
        dgdx0(1,:) = tmp(1:6);
        
        [v,grad] = dlfeval(@model,nny,input_sample);
        tmp = extractdata(grad); % input_sample = extractdata(input_sample);
        dgdx0(2,:) = tmp(1:6);

        [v,grad] = dlfeval(@model,nnr,input_sample);
        tmp = extractdata(grad); % input_sample = extractdata(input_sample);
        dgdx0(3,:) = tmp(1:6);
